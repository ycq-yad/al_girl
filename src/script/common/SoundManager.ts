...
* 音频管理类
*/
export default class Als_SoundManager {

    constructor() {
    }

    private static instance: Als_SoundManager;
    public static getInstance(): Als_SoundManager {
    ...
    public destoryOneSound(soundName: string): void {
        let _url = Als_SoundConst.getKeyUrl(soundName);
    ...
    public async playEffect(soundUrl: string, times: number) {
        let _url = Als_SoundConst.getKeyUrl(soundUrl);
    ...
    public playMiniGameEffect(soundUrl: string): void {
        if (DeviceUtil.isWXMiniGame() || DeviceUtil.isQQMiniGame()) {
            ...
            if (!innerAudioContext) {
                Als_SoundManager.getInstance().effectPools[soundUrl] = innerAudioContext = platform.createInnerAudioContext();
...
class MiniGameSound {
    ...
    public create(soundUrl: string): void {
        ...
        this.innerAudioContext.onEnded(() => {
            this.isEnded = true;
            Als_SoundManager.getInstance().effectPools[this.soundUrl].push(this);
