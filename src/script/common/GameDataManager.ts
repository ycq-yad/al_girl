...
import { Als_GameData } from "./GameData";

export class Als_PlayerDataBase {
    nMaxLevel: number;
    nCurLevel: number;
    nCurIndex: number;
...
export class PlayerDataManager_Als {

    private static instance: PlayerDataManager_Als;
    public static getInstance(): PlayerDataManager_Als {
        if (!this.instance) {
    ...
    /**新增看视频无限体力功能数据 */
    private stNewFuncPsLimitless: Als_NewFuncPsLimitless;
    ...
    public stPlayerDataBase: Als_PlayerDataBase;

    public stDYLogData: Als_DYLogData;
    ...
    public SaveData() {
        ...
        let stra = Laya.LocalStorage.getItem(Als_GameData.getInstance().userInfo.openId + "BaseData");
    ...
    /**获取数据*/
    public GetData() {
        let str = Laya.LocalStorage.getItem(Als_GameData.getInstance().userInfo.openId + "BaseData");
        if (str) {
            try {
        ...
        let strOperData0807 = Laya.LocalStorage.getItem("OperData0807" + Als_GameData.getInstance().userInfo.openId);
        if (strOperData0807) {
    ...
    public AddGoods(nType: number, nCount: number) {
        if (nType == GoodsType.enum_GoodsType_Glod) {
            ...
            let nMax = 99;
            let stGameConfig = ConfigManager_Als.getInstance().getGameConfigDataByID(4)
            if (stGameConfig) {
    ...
    public addPsAboutOffLine() {
        ...
        let nMaxPs = 10;
        let stGameConfig = ConfigManager_Als.getInstance().getGameConfigDataByID(1);
        if (stGameConfig) {
        ...
        if (stGameConfig) {
            ...
            let nCurtTime = GameLogicProcessingManager_Als.GetCurTime();
    ...
    private openPSRecoveryTime() {
        ...
        if (this.nPsRecoveryTime == 0) {
            let stGameConfig = ConfigManager_Als.getInstance().getGameConfigDataByID(2);
            if (!stGameConfig) {
                return;
            }
        ...
        if (this.stPlayerDataBase.nPS >= nMaxPs) {
            ...
            if (!GameLogicProcessingManager_Als.getInstance().PSRecoveryOpen) {
                ...
                let nCurTime = GameLogicProcessingManager_Als.GetCurTime();
                if (this.stPlayerDataBase.nPsTime == 0) {
    ...
    public initGoods() {
        ...
        //初始话体力
        let stGameConfig = ConfigManager_Als.getInstance().getGameConfigDataByID(15);
        if (stGameConfig) {
