...
import GuessItemData_Als from "./GuessItemData";
 ...
 */
export default class GuessLike_Als extends BaseSceneUISkin {
     ...
     * 数据源
     */
    public listdata: GuessItemData_Als[] = [];
    private subItemSkin: string;

    constructor(skin: string, subItemSkin: string, listdata_: GuessItemData_Als[], itemW: number) {
        super();
    ...
    private initList(): void {
        ...
        for (let i = 0, len = this.listdata.length; i < len; i++) {
            let item = new GuessLikeItem_Als(this.subItemSkin, this.listdata[i]);
    ...
    private frameChange(): void {
        ...
        if (curX > (self.index + 1) * self.itemW) {
            self.index++;
            let last = self.boxView.removeChildAt(self.boxView.numChildren - 1) as GuessLikeItem_Als;
            let first = (self.boxView.getChildAt(0) as GuessLikeItem_Als);
        ...
        if (curX < (self.index - 1) * self.itemW) {
            self.index--;
            let last = (self.boxView.getChildAt(self.boxView.numChildren - 1) as GuessLikeItem_Als);
            let first = self.boxView.removeChildAt(0) as GuessLikeItem_Als;
            first.x = last.x + self.itemW;
            self.boxView.addChild(first);
            first.exposure();
            return
