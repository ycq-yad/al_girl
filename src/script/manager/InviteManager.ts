 ...
 * 关联邀请人数据管理
 */
export default class InviteManager_Als {

    private static instance_: InviteManager_Als;
    public static getInstance(): InviteManager_Als {
    ...
    /**请求连接 */
    private URL: string = Als_GameData.getInstance().URL_OF_INVITE;

    /** 邀请人信息 */
    public inviterInfo: netData.Als_Inviter = new netData.Als_Inviter();
    ...
    public selectInfo(callF: Function = null, obj: any = null): void {
        let gameId: string = Als_GameData.getInstance().gameId;
        let openId: string = Als_GameData.getInstance().userInfo.openId;
        let msg: any = {};
        msg.msg_type = "16";
        msg.msg_data = {
            "gameid": gameId,
    ...
    public async createInfo(callF: Function = null, obj: any = null) {
        await new Promise((res, rej) => {
            ...
            let tx_url: string = Als_GameData.getInstance().userInfo.avatarUrl;
            let nick: string = Als_GameData.getInstance().userInfo.nick;
            let gameId: string = Als_GameData.getInstance().gameId;
            let msg: any = {};
            msg.msg_type = "14";
            msg.msg_data = {
    ...
    public judgeInvite() {
        return new Promise(resolve => {
            let res = Als_GameData.getInstance().enterGameInfo;
            ...
            if (res) {
                ...
                if (scene == 1007 || scene == 1008 || scene == 1044) { // 1007:好友分享 1008:群分享
                    if (Als_GameData.getInstance().userInfo.openId && res.query && res.query["openid"]) {
                        ...
                        if (Als_GameData.getInstance().userInfo.openId != this.inviterInfo.openId) { // 排除自己邀请自己的情况
    ...
    /**得到邀请信息 */
    public getInviteAwardData(): localData.Als_InviteData[] {
        let inviteConfig = ConfigManager_Als.getInstance().getInviteConfigInfo();
        let lingqu = PlayerDataManager_Als.getInstance().stPlayerDataBase.inviteId;
        ...
        for (let i = 0, len = inviteConfig.length; i < len; i++) {
            ...
            let data = new localData.Als_InviteData();
            data.id = awardId;
