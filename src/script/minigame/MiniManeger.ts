...
import { GoodsType, MoreGameIndex_Als } from "../games/CommonDefine";
 ...
 * 小游戏管理器
 */
export class MiniManeger_Als {
    private static ins: MiniManeger_Als;
    public static get instance(): MiniManeger_Als {
  ...
  */
    ...
    */
    ...
    public playViderAd(data: { successFun?: Function, failFun?: Function, errorFun?: Function, isLongVideo?: boolean }) {
        ...
        let videoId = Als_GameData.getInstance().videoId;
        if (data.isLongVideo) {
    ...
    public showBannerAd(offset?: { w: number, h: number, callback?: Function }) {
        ...
        let bannerId = Als_GameData.getInstance().bannerId;
    ...
    public addOpenWxData(data: { x?: number, y?: number, width: number, height: number, left?: number, right?: number, top?: number, bottom?: number, parent: Laya.Sprite, isCenter?: boolean }) {
        let shareData = MiniManeger_Als.instance.getShareInfo({ id: Als_GameData.getInstance().userInfo.openId })
    ...
    public showMoreGamesModal(): void {
        ...
        for (let i = 0, len = Als_GameData.getInstance().weCatMiniIconsInfo.length; i < len; i++) {
     ...
     * 创建猜你喜欢
     */
    public async createGuessLike(parent: Laya.Sprite): Promise<GuessLike_Als> {
        return new Promise<GuessLike_Als>(async (resolve) => {
            ...
            guessLike = parent.getChildByName('GuessLike') as GuessLike_Als;
            if (guessLike == null) {
                //2020.7.13-3
    ...
    public static saveOpenID() {
        ...
        let openId: string = Als_GameData.getInstance().strOpenIDWx;
